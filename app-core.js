...
function applyData(d){
  ...
  document.getElementById('whoH').innerHTML=UICON.user+'<b>'+esc(ME.name)+'</b>（'+esc(ME.role)+'）';
...
function chips(a){let h='';if(a.over)h+=`<span class="chip red">${sev('超過')} 超過 ${a.over}</span>`;if(a.soon)h+=`<span class="chip amber">${sev('間近')} 返却間近 ${a.soon}</span>`;if(a.longg)h+=`<span class="chip warn">${sev('長期')} 長期確認 ${a.longg}</span>`;return h||'<span class="note">アラートなし</span>';}
...
// 絵文字を廃した単色SVGアイコン群。sev()は重大度（色が意味を持つので明示色＋白の！）。
const SEVCOL={'超過':'#ef4444','間近':'#f59e0b','長期':'#f97316'};
const sev=a=>SEVCOL[a]?`<svg class="sev" viewBox="0 0 16 16" style="color:${SEVCOL[a]}"><circle cx="8" cy="8" r="6.5" fill="currentColor"/><path d="M8 4.6v4.3" stroke="#fff" stroke-width="1.7" stroke-linecap="round"/><circle cx="8" cy="11.6" r=".95" fill="#fff"/></svg>`:'';
const UICON={
  user:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="3.4"/><path d="M5.5 20a6.5 6.5 0 0 1 13 0"/></svg>',
  searchBig:'<svg class="gbig" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="m21 21-4-4"/></svg>'
};
...
function parseRoute(){let path=location.pathname;if(path.indexOf(BASE)===0)path=path.slice(BASE.length);
  ...
  if(segs[0]&&/^(index|404)\.html$/i.test(segs[0]))segs.shift();   // /index.html・/404.html はルート(/)と同等に扱う
  const sec=!segs[0]?'dashboard':(SECTIONS.indexOf(segs[0])>=0?segs[0]:'notfound');   // 未知パスは notfound（404表示）
...
function renderNotFound(){document.getElementById('main').innerHTML=`<div class="empty"><div>${UICON.searchBig}<div style="font-weight:700;font-size:16px;color:var(--ink)">ページが見つかりません</div><div class="note" style="margin-top:6px">お探しのURLは存在しないか、変更された可能性があります。</div><div style="margin-top:16px"><button class="btn primary" onclick="go('dashboard')">ダッシュボードへ戻る</button></div></div></div>`;}
...
function renderProduct(){if(!selected){document.getElementById('main').innerHTML=`<div class="empty"><div>${UICON.searchBig}左の製品ツリー、または検索ボックスから<br>商品を選んでください。</div></div>`;return;}
...
function renderGantt(){const us=unitsOf(selected),total=days(WIN_START,WIN_END),W=total*zoomPx,x=dt=>days(WIN_START,dt)*zoomPx,todayX=x(TODAY);
  ...
  us.forEach(u=>{const s=statusOf(u),l=u.loan,active=l&&l.shipped&&!l.returned&&!l.hold,clk=(s.key==='貸出中'||isHoldS(s.key)||s.key==='出荷待ち');
    ...
    if(active){const start=l.ship<WIN_START?WIN_START:l.ship,undated=l.dueType!=='日付指定',end=undated?WIN_END:(l.due>WIN_END?WIN_END:l.due),left=x(start),w=Math.max(x(end)-left,10);
      ...
      const tag=s.alert?' '+sev(s.alert):'';const dueTxt=l.dueType==='日付指定'?fmtY(l.due)+'まで':l.dueType;
...
function openShipCase(key){if(!canField()){alert('出荷確定は事務所・倉庫担当のみ可能です。');return;}const c=waitingCases().find(x=>x.key===key);if(!c)return;const l=c.loan;shipCaseKey=key;
  ...
  document.getElementById('caseCard').innerHTML=`<h3>出荷確定</h3>
   ...
   <div class="scnote">色の付いた欄が倉庫の記入項目です。塗ってある所だけ記入してください（それ以外は受付内容で変更できません）。</div>
   ${rowsHtml}
